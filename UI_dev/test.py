...
class PowerMeterApp:
    def __init__(self, root):
        ...
        recording_duration_visible = tk.IntVar()
    ...
    def afficher_masquer_bouton():
        """
        creates the duration widgets if in recording mode
        """
        # Si la case est cochée (valeur=1), afficher le bouton
        if self.option_activee.get() == 1:
            self.bouton_conditionnel.grid(row=1, column=0, pady=10, padx=10)
        else:
            # Sinon, cacher le bouton
            self.bouton_conditionnel.grid_forget()
